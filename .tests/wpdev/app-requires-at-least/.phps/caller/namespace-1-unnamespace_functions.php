...
namespace MyProject {
    // call functions test. =================================================
    connect();// uqn, 
    // expect `MyProject\connect()` if there is no this function under "unnamespace", 
    // expect `connect()` if there is this function under "unnamespace".
    \date('Y-m-d');// resolution fqn
    ...
    global $greet;
    $greet('Rose');// expect anonymous `$greet()`.

    echo call_user_func_array('manualWriteFunctionNameInCallUserFuncArray', []);// fqn
