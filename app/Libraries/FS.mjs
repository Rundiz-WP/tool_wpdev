...
export default class FS {
    ...
    /**
     * Copy files and folders to destination use `fs.cpSync()`. By default, it is copy all sub folder recursively.
     * 
     * @param {string} source Source path to copy.
     * @param {string} destination Destination path to copy to.
     * @param {object} options The node `fs.cpSync` options. See https://nodejs.org/dist/latest-v18.x/docs/api/fs.html#fscpsyncsrc-dest-options
     * @returns {void}
     */
    static cpSync(source, destination, options = {}) {
        if (typeof(source) !== 'string') {
            throw new Error('The source argument must be string.');
        }

        if (typeof(destination) !== 'string') {
            throw new Error('The destination argument must be string.');
        }
    ...
    /**
     * This will be use `deleteAsync()` function of `del` package.
     * 
     * This is for external class that cannot be import `del` package directly.
     * 
     * @param {string|string[]} patterns See the supported [glob patterns](https://github.com/sindresorhus/globby#globbing-patterns).
     * @param {Object} options You can specify any of the [`globby` options](https://github.com/sindresorhus/globby#options) in addition to the `del` options.
     * @returns The deleted paths.
     */
    static async deleteAsync(patterns, options) {
        return await deleteAsync(patterns, options);
    }// deleteAsync
    ...
    static watch(pattern, options = {}) {
        ...
        const filesList = fs.globSync(pattern);
